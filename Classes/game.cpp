...
bool New::init()
{
	...
	////////////신발
	l_Shose = Label::create(StringUtils::format("%.0f : %.0f ", ShoseTime / 60, (int)ShoseTime % 60), "fonts/NanumBarun/NanumBarun.ttf", 24);
	if (UserDefault::getInstance()->getIntegerForKey("Have_Shose") == false)
		have_Shose = 5;
	else 
		have_Shose = UserDefault::getInstance()->getIntegerForKey("Have_Shose");
	if (UserDefault::getInstance()->getFloatForKey("Shose_Time") == false)
		ShoseTime = 600;
	else 
		ShoseTime = UserDefault::getInstance()->getFloatForKey("Shose_Time");
	for (int i = 0; i < have_Shose; i++)
	{
		Shose[i] = Sprite::create("Texture/Object/UI/Shose.png");
		Shose[i]->setPosition(800 + (100 * i), 650);
		Shose[i]->setScale(0.5f);
		Shose[i]->setAnchorPoint(Point(0.5f, 0.5f));
		this->addChild(Shose[i], 2);
	}
	...
	Alice1 = Sprite::create("Rust/D1.png");
	Alice1->setPosition(Vec2(370, 350));
...
bool New::onTouchBegan(Touch* touch, Event* _event)
{
	...
	if (Playbt->getBoundingBox().containsPoint(touch->getLocation()) && bisClick == true && StageClick == false)
	{
		have_Shose -= 1;
		UserDefault::getInstance()->setIntegerForKey("Have_Shose", have_Shose);
